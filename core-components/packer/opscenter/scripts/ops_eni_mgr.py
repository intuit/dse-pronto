...
import pprint
import time
from ops_ec2_mgr import OpsEC2Manager
...
class CasNetworkInterfaceManager:
    ...
    def __init__(self, region):
        ...
        self._ec2_mgr = OpsEC2Manager(self._session, self._client, self._resource)
        self._pp = pprint.PrettyPrinter(indent=4)

    def get_hosts(self, node_list, match_by_name):
        return self._ec2_mgr.get_hosts(node_list, match_by_name)
    ...
    def get_enis(self,operation, nodes):
        """
            Inputs
    ...
    def attach_enis(self, eni_list, hosts):
        self.attach_detach_enis(eni_list, hosts, "attach")

    def detach_enis(self, eni_list, hosts):
        self.attach_detach_enis(eni_list, hosts, "detach")

    def get_host_for_eni(self, eni, hosts, operation):
        ...
        for t in eni["tags"]:
            if t["Key"] == "Name":
                eni_name = t["Value"]
        host = None
        matching_hosts = [x for x in hosts if x["name"] == eni_name]
    ...
    def attach_detach_enis(self, eni_list, hosts, operation):
        attach_ids = []
        for eni in eni_list:
            host = self.get_host_for_eni(eni, hosts, operation)
...
if __name__ == "__main__":
    ...
    eni_list = casNICMgr.get_enis(options.operation, node_list)
    ...
    if options.operation == "attach":
        ...
        hosts = casNICMgr.get_hosts(node_list, True)
        ...
        hosts = casNICMgr.get_hosts(node_list, False)
